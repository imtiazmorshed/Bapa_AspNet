...
namespace OMS.DAL
{
    ...
    {
        private const string ConnectionStringName = "OMSConnectionString";

        public static OMSDataContext GetData()
        {
            var db = new OMSDataContext(ConfigurationManager.ConnectionStrings[ConnectionStringName].ConnectionString); 
            return db;
        }
        ...
        public static void UpdateAll<T>(List<T> items, Action<OMSDataContext, T> update) where T : class
        {
            var Database = DatabaseHelper.GetData();
