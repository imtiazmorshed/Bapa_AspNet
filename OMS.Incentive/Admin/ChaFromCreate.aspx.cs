...
{
    public partial class ChaFromCreate : System.Web.UI.Page
    {
        ...
        public void LoadChaFormDetails()
        {
            ...
            else
            {
                using (TheFacade facade = new TheFacade())
                {
                    ...
                    MemberID = chaForm.MemberID;
                    ddlMember.SelectedValue = chaForm.MemberID.ToString();
        ...
        protected void btnInvoiceSave_Click(object sender, EventArgs e)
        {
            ...
            long selectedInvID = Convert.ToInt32(ddlInvoiceList.SelectedValue);
            if (!invoiceIds.Exists(delegate (long id) { return id == selectedInvID; }))
            {
